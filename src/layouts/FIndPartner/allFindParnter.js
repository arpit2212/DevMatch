...
import FindPartnerData from '../../mockData/FindPartner.js';

const AllFindPartner = () => {
  const [search, setSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // Filter the partners based on the search input
  const filteredPartners = FindPartnerData.filter((partner) =>
  ...
  const currentItems = filteredPartners.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="bg-gradient-to-r from-white via-gray-100 to-white min-h-screen p-6 relative overflow-hidden mt-12">
      ...
      <h2 className="text-4xl font-extrabold text-center text-indigo-700 mb-10 drop-shadow-md">All Partners</h2>
      ...
      {filteredPartners.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No Partner Found</p>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
            {currentItems.map((partner) => (
              ...
              >
                ...
                />
                <h3 className="text-2xl font-bold mb-2 text-indigo-700">{partner.title}</h3>
                <p className="text-black mb-3">{partner.description}</p>
                <p className="text-sm text-black mb-1">Location: {partner.location}</p>
                <p className="text-sm text-gray-600">Date Posted: {partner.datePosted}</p>
                <div
